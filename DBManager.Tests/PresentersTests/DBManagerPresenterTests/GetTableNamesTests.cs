...
public class GetTableNamesTests : DBManagerTestBase
{
    [Fact]
    public async Task ReturnsOk_WhenConnectionExists()
    {
        ...
        var enginePresenter = MockEnginePresenter
             .Init(connection)
             .WithOk_GetTableNames(new TableNamesResponseDto { Names = names })
             .GetSuccessful();
    ...
    public async Task ReturnsError_WhenConnectionNotExist()
    {
        ...
        var response = await Act(connection.Name, "some-database-name");

        ResponseAssert.Error(response);
    }

    private Task<Response<TableNamesResponseDto>> Act(
        ...
        IEngineModuleResolver engineModuleResolver = null)
    {
        ...
        return sut.GetTableNames(connectionName, databaseName);
    }
}
